...
# ==========================================
# 👨‍✈️ HAYDOVCHI HANDLERS
# ==========================================

@driver_dp.message(F.location)
async def dr_loc_update(message: types.Message):
    """Bu funksiya harakat davomida masofani hisoblaydi"""
    lat, lon = message.location.latitude, message.location.longitude
    did = message.from_user.id
    conn = sqlite3.connect(DB_FILE)
    
    # Haydovchining onlayn lokatsiyasini yangilash
    conn.execute("UPDATE drivers SET lat=?, lon=? WHERE user_id=?", (lat, lon, did))
    
    # Safar ketayotgan bo'lsa masofani hisoblash
    ...
    if tr and tr[0] == 1:
        ...
        if last_lat > 0:
            ...
            if 0.01 < step < 0.5: # 10 metrdan 500 metrgacha bo'lgan harakatlarni qo'shish
                new_dist = total_dist + step
                conn.execute("UPDATE trips SET total_dist=?, last_lat=?, last_lon=? WHERE driver_id=?", (new_dist, lat, lon, did))
        else:
...
async def ride_start(call: CallbackQuery):
    ...
    # Ojidaniyani to'xtatib safarni boshlash
    ...
    conn.commit(); conn.close(); await call.answer("Safar va masofa hisobi boshlandi!")

# --- QOLGAN FUNKSIYALAR (ARRAIVED, FINISH, REG VA HKZ) ---
...
async def dr_start(message: types.Message, command: CommandObject, state: FSMContext):
    ...
    if not user: await state.set_state(DriverReg.name); await message.answer("Ismingizni kiriting:")
    else:
        kb = ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="🟢 Ishni boshlash (Live)", request_location=True)]], resize_keyboard=True)
        await message.answer("Live Location yuboring.", reply_markup=kb)
...
async def arrived(call: CallbackQuery):
    ...
    await call.message.edit_text("Yetib keldingiz. Mijoz chiqsa 'Safarni boshlash' bosing.")
...
async def wait_play(call: CallbackQuery):
    conn = sqlite3.connect(DB_FILE); conn.execute("UPDATE trips SET wait_start=? WHERE driver_id=?", (time.time(), call.from_user.id)); conn.commit(); conn.close(); await call.answer("Ojidaniye yoqildi")
...
async def wait_pause(call: CallbackQuery):
    ...
    if tr and tr[0] > 0:
        ...
        conn.commit(); conn.close(); await call.answer("Pauza")
...
async def finish(call: CallbackQuery):
    ...
    if tr:
        ...
        conn.execute("DELETE FROM trips WHERE driver_id=?", (call.from_user.id,)); conn.commit(); conn.close()

# --- QABUL/SKIP VA MIJOZ BOTI ---
...
async def start_trip_logic(did, cid, cph, lat, lon, msg=None):
    ...
    c_msg = await client_bot.send_message(cid, f"🚕 Haydovchi: {d[0]}\n🚗 {d[2]} ({d[3]})"); ikb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🚕 Yetib keldim", callback_data="arrived")]])
    ...
    conn.execute("INSERT OR REPLACE INTO trips (driver_id, client_id, client_phone, driver_msg_id, client_msg_id) VALUES (?,?,?,?,?)", (did, cid, cph, d_msg.message_id, c_msg.message_id)); conn.execute("UPDATE drivers SET status='busy' WHERE user_id=?", (did,)); conn.commit(); conn.close()
...
async def client_final(message: types.Message, state: FSMContext):
    d = await state.get_data(); station = find_station(d['lat'], d['lon']); conn = sqlite3.connect(DB_FILE); dr = conn.execute("SELECT user_id FROM drivers WHERE status='online' AND station=? LIMIT 1", (station,)).fetchone(); conn.close()
    if dr:
