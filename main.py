...
async def taximeter_loop(did):
    while True:
        ...
        # Masofani hisoblash (Faqat Safar boshlangan bo'lsa)
        curr_dist = total_dist
        if is_riding == 1:
            dr = conn.execute("SELECT lat, lon FROM drivers WHERE user_id=?", (did,)).fetchone()
            if dr and last_lat > 0:
                d = get_dist(last_lat, last_lon, dr[0], dr[1])
                if d < 0.3: # To'g'ri hisoblash uchun
                    curr_dist += d
                    conn.execute("UPDATE trips SET total_dist=?, last_lat=?, last_lon=? WHERE driver_id=?", (curr_dist, dr[0], dr[1], did))
                    conn.commit()
        ...
        if is_riding == 0:
            if wait_start > 0:
                btns.append([InlineKeyboardButton(text="⏸ Pauza", callback_data="wait_pause")])
            else:
                btns.append([InlineKeyboardButton(text="▶️ Ojidaniya (Play)", callback_data="wait_play")])
...
async def arrived(call: CallbackQuery):
    ...
    await call.message.edit_text("Yetib keldingiz. Ojidaniya yoqildi.", reply_markup=ikb)
...
async def wait_play(call: CallbackQuery):
    ...
    conn.execute("UPDATE trips SET wait_start=? WHERE driver_id=?", (time.time(), call.from_user.id))
    conn.commit(); conn.close()
    await call.answer("Ojidaniya davom etmoqda")
