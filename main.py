...
# ==========================================
# 🔄 ASOSIY TAXIMETER (TUGMALARNI BOSHQARISH)
# ==========================================
...
async def taximeter_loop(did):
    while True:
        ...
        # Kutish vaqtini hisoblash (bu masofaga dahilsiz)
        curr_wait = total_wait_min + ((time.time() - wait_start) / 60 if wait_start > 0 else 0)
        
        # Narx: Minimalka + (masofa > 1km bo'lsa har km ga 1000) + kutish
        ...
        if wait_start > 0:
            ...
            kb.append([InlineKeyboardButton(text="▶️ Kutishni boshlash", callback_data="wait_play")])
...
async def dr_location_handler(message: types.Message):
    ...
    # Faqat Live Location orqali online bo'ladi
    if message.location.live_period:
        ...
        if curr and curr[0] != 'busy':
            ...
            await message.answer(f"📍 {st}\n👥 Navbat: {pos}-chi", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔴 Offline bo'lish", callback_data="go_offline")]]))

    # Masofa hisoblash (is_riding == 1 bo'lsagina ishlaydi)
    ...
    if tr and tr[0] == 1:
        ...
        if l_lat > 0:
            ...
            if 0.005 < step < 0.6: # GPS xatoliklarisiz masofani qo'shish
...
async def arrived_cb(call: CallbackQuery):
    ...
    conn.commit(); conn.close()
    await call.message.edit_text("Yetib keldingiz. Kutish hisoblanmoqda...")
...
async def ride_start_cb(call: CallbackQuery):
    conn = sqlite3.connect(DB_FILE)
    # Masofa hisobini boshlash uchun koordinatalarni yangilaymiz
...
async def play_cb(call: CallbackQuery):
    conn = sqlite3.connect(DB_FILE); conn.execute("UPDATE trips SET wait_start=? WHERE driver_id=?", (time.time(), call.from_user.id)); conn.commit(); conn.close(); await call.answer("Kutish boshlandi")

@driver_dp.callback_query(F.data == "fin_pre")
async def finish_cb(call: CallbackQuery):
...
# --- STANDART QISMLAR (REG, START, BUYURTMA) ---
...
async def start_trip_logic(did, cid, cph, lat, lon, msg=None):
    ...
    d_msg = await msg.edit_text(f"✅ Buyurtma: {cph}", reply_markup=ikb) if msg else await driver_bot.send_message(did, f"✅ Buyurtma: {cph}", reply_markup=ikb)
    # Mijoz uchun bo'sh xabar (loop yangilab turishi uchun)
    c_msg = await client_bot.send_message(cid, "🚕 Safar ma'lumotlari kutilmoqda...")
...
async def off_cb(call: CallbackQuery):
    conn = sqlite3.connect(DB_FILE); conn.execute("UPDATE drivers SET status='offline' WHERE user_id=?", (call.from_user.id,)); conn.commit(); conn.close()
    await call.message.edit_text("🔴 Offline holatdasiz.")
...
async def dr_n(message, state): await state.update_data(n=message.text); await state.set_state(DriverReg.phone); await message.answer("📞 Raqam:", reply_markup=ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="📱 Raqam", request_contact=True)]], resize_keyboard=True))
@driver_dp.message(DriverReg.phone, F.contact)
async def dr_p(message, state): await state.update_data(p=message.contact.phone_number); await state.set_state(DriverReg.car_model); await message.answer("🚗 Mashina:", reply_markup=ReplyKeyboardRemove())
@driver_dp.message(DriverReg.car_model)
async def dr_m(message, state): await state.update_data(m=message.text); await state.set_state(DriverReg.car_number); await message.answer("🔢 Raqami:")
@driver_dp.message(DriverReg.car_number)
async def dr_f(message, state):
...
async def c_start(message): await message.answer("📍 Lokatsiya yuboring:", reply_markup=ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="📍 Lokatsiya", request_location=True)]], resize_keyboard=True))
@client_dp.message(F.location)
async def c_loc(message, state): await state.update_data(lat=message.location.latitude, lon=message.location.longitude); await state.set_state(ClientOrder.waiting_phone); await message.answer("📱 Raqam:", reply_markup=ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="📱 Raqam", request_contact=True)]], resize_keyboard=True))
@client_dp.message(ClientOrder.waiting_phone, F.contact)
async def c_final(message, state):
