...
# ==========================================
# 🔄 ASOSIY TAXIMETER LOOP (MASOFA VA VAQT)
# ==========================================
...
async def taximeter_loop(did):
    while True:
        ...
        # 2. Masofani yangilash (GPS orqali)
        curr_dist = total_dist
        dr = conn.execute("SELECT lat, lon FROM drivers WHERE user_id=?", (did,)).fetchone()
        
        if dr and dr[0] > 0:
            if is_riding == 1 and last_lat > 0:
                step_dist = get_dist(last_lat, last_lon, dr[0], dr[1])
                # Sakrashni oldini olish (agar masofa juda katta bo'lsa yoki 0 bo'lsa hisoblamaydi)
                if 0.005 < step_dist < 0.5: 
                    curr_dist += step_dist
            
            # Har doim oxirgi nuqtani yangilaymiz (masofa hisoblash davom etishi uchun)
...
# ==========================================
# 👨‍✈️ HAYDOVCHI BOTI (FULL REG + LIVE LOCATION)
# ==========================================
...
async def dr_final(message: types.Message, state: FSMContext):
    ...
    conn.execute("INSERT OR REPLACE INTO drivers (user_id, name, phone, car, car_num, status) VALUES (?,?,?,?,?,'online')", 
...
async def dr_loc_update(message: types.Message):
    if message.location.live_period:
        ...
        await message.answer("⚠️ Iltimos, faqat 'Share Live Location' (8 soatlik) yuboring.")

# --- SAFAR BOSHQARUVI CALLBACKS ---

@driver_dp.callback_query(F.data == "arrived")
async def arrived(call: CallbackQuery):
    conn = sqlite3.connect(DB_FILE)
    # Yetib kelganda avtomatik ojidaniye boshlanadi
    conn.execute("UPDATE trips SET wait_start=? WHERE driver_id=?", (time.time(), call.from_user.id))
    conn.commit(); conn.close()
    
    await call.message.edit_text("Siz yetib keldingiz. Mijoz chiqishi bilan 'Safarni boshlash'ni bosing.")
...
async def ride_start(call: CallbackQuery):
    ...
    # Ojidaniyani to'xtatib, riding rejimini yoqish
    new_wait = tr[1] + ((time.time() - tr[0])/60 if tr[0] > 0 else 0)
    conn.execute("UPDATE trips SET wait_start=0, total_wait=?, is_riding=1, last_lat=?, last_lon=? WHERE driver_id=?", 
...
async def finish_trip(call: CallbackQuery):
    ...
    if tr:
        ...
        res = f"🏁 Safar yakunlandi!\n\n⏱ Ojidaniye: {int(w_min)} daq\n🛣 Masofa: {dist:.2f} km\n💰 Jami to'lov: {int(total)} so'm"
...
async def skip_order(call: CallbackQuery):
    ...
    await call.message.edit_text("🔄 Buyurtma rad etildi."); await find_and_send_driver(int(cid), "Mijoz", cph, float(lat), float(lon), exclude_id=call.from_user.id)
...
async def start_trip_logic(did, cid, cph, lat, lon, msg=None):
    ...
    d_msg = await msg.edit_text(f"✅ Buyurtma qabul qilindi. 📞 {cph}", reply_markup=ikb) if msg else await driver_bot.send_message(did, f"✅ Buyurtma qabul qilindi. 📞 {cph}", reply_markup=ikb)
